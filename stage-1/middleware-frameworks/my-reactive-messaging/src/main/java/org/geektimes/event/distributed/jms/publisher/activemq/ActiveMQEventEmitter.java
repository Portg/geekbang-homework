package org.geektimes.event.distributed.jms.publisher.activemq;

import org.apache.activemq.ActiveMQConnection;
import org.apache.activemq.ActiveMQConnectionFactory;
import org.geektimes.event.distributed.jms.AbstractJmsEvent;
import org.geektimes.event.distributed.jms.Destination;
import org.geektimes.event.distributed.jms.publisher.JmsEventEmitter;

import javax.jms.Connection;
import javax.jms.JMSException;
import java.net.URI;
import java.net.URISyntaxException;
import java.util.*;


public abstract class ActiveMQEventEmitter<E extends AbstractJmsEvent<?>> extends JmsEventEmitter<E> {

    @Override
    public Connection getOpenConnection(Properties properties) throws JMSException {
        ActiveMQConnectionFactory factory = null;
        try {
            factory = new ActiveMQConnectionFactory(buildRealEndpointURI(properties));
        } catch (URISyntaxException e) {
            throw new RuntimeException(e);
        }
        ActiveMQConnection conn = (ActiveMQConnection) factory.createConnection();
        conn.start();
        return conn;
    }

    protected URI buildRealEndpointURI(final Properties properties) throws URISyntaxException {
        String host = this.properties.getProperty(HOST_PROPERTIES, "localhost");
        int port = -1;
        try {
            port = Integer.parseInt(this.properties.getProperty(PORT_PROPERTIES, "0"));
        } catch (NumberFormatException ignored) {
        }
        if (port <= 0)
            port = 61616;
        return new URI(String.format("tcp://%s:%d", host, port));
    }

    @Override
    protected Set<Destination> getSupportedDestinations() {
        return Collections.unmodifiableSet(new LinkedHashSet<>(
                Arrays.asList(Destination.fromActiveMQ())
        ));
    }

    private final static String HOST_PROPERTIES = "jms.activemq.endpoint.host";
    private final static String PORT_PROPERTIES = "jms.activemq.endpoint.port";

}
